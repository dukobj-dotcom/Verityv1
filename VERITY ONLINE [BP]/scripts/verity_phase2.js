...
import { playerHoldingVerity, syncHeldVerityItem, VERITY_INVENTORY_IDS } from "./verity_items.js";
import {
	isStoryModeEnabled,
	ensureSandboxAnchor,
	SANDBOX_ANCHOR_PROP,
...
/** Sau hơn 8 lần giết verityball → phase 2 ngay (lần thứ 9). */
...
function sendCountdownAnnouncementOnce() {
	...
	// Verity te llama ("Ey...") y se pone rara al entrar la fase.
	for (const p of world.getPlayers()) {
		const b = findNearestBall(p);
		if (b?.isValid) {
			playVerityVoice(b, VOICE.EY_INICIO);
			break;
		}
	}
	sendVerityChat("Algo viene en 3 días.");
}
...
function pickLine(key) {
	...
	const lines = {
		...
		how_are_you: [
			"Bien. ¿Tú?",
			"Sigo aquí. Sigo observando.",
			"Podría estar peor. Podría ser tú.",
		],
		greet: ["Hey.", "Hello.", "Hola. Sigo aquí."],
		thanks: ["Sure.", "Anytime.", "No lo menciones."],
		mumble: ["Sigo aquí.", "Sigue hablando.", "Te oigo.", "Mhm."],
		ignore: [
			"...",
			"No te oí.",
			"Pregunta de nuevo. O no.",
		],
	};
...
function runTrappedSequence(player, ball, reason) {
	...
	system.runTimeout(() => {
		...
		playTrappedVoice(ball, player, VOICE.NO_ESCONDERTE);
		sendVerityChat("No puedes esconderte de mí.");
	}, freeAt);
}
...
export function tryPhase2Chat(player, message, ball) {
	...
	if (state === P2_STATE.BORED && phase === PHASE.TWO && asksAboutFace(message)) {
		...
		sendVerityChat("Estoy sonriendo ahora.");
	...
	if (
		...
		if (resolvedBall) {
			...
			if (count > 0) {
				const lines = ["Done.", "There.", "Todos ellos."];
				return {
		...
		return { text: "No hay puertas cerca.", intent: "story", animate: true };
	}
	...
	// Si en la cuenta regresiva le preguntas si está bien / cómo está,
	// responde "estoy bien" con su voz (antes del catch-all genérico).
	if (state === P2_STATE.COUNTDOWN || state === P2_STATE.COUNTDOWN_DAY2) {
		const n = expandMessage(normalizeQuestion(message));
		if (
			/\b(are you okay|are you ok|you alright|how are you|you good|feeling okay)\b/.test(n)
		) {
			return {
				text: "Qué lindo que preguntes. Estoy bien.",
				intent: "story",
				animate: true,
				voice: VOICE.ESTOY_BIEN_FASE,
			};
		}
	}

	// En la fase de cuenta regresiva, cualquier otra cosa que preguntes
	// recibe siempre "Algo viene en 3 días." (texto + voz).
	if (state === P2_STATE.COUNTDOWN || state === P2_STATE.COUNTDOWN_DAY2) {
		return {
			text: "Algo viene en 3 días.",
			intent: "story",
			animate: true,
			voice: VOICE.ALGO_VIENE,
		};
	}
...
export function initVerityPhase2() {
	...
	if (beforeSleep) {
		beforeSleep.subscribe((ev) => {
			...
			ev.cancel = true;
			sendVerityChat("Verity está cerca. No puedes dormirte.");
		});
	}
