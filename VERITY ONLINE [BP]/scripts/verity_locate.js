...
export function parseLocateCoords(raw) {
	...
	const patterns = [
		...
		// Español (Bedrock): "está en el bloque -504, (y?), 312"
		/bloque\s+(-?\d+)\s*,\s*\(?[^,)]*\)?\s*,\s*(-?\d+)/i,
		// Inglés con (y?): "at block -504, (y?), 312"
		/block\s+(-?\d+)\s*,\s*\(?[^,)]*\)?\s*,\s*(-?\d+)/i,
		/(-?\d+)\s*,\s*(-?\d+)\s*,\s*(-?\d+)/,
		/(-?\d+)\s+(-?\d+)\s+(-?\d+)/,
	];
	...
	for (const pattern of patterns) {
		...
		if (!match) continue;
		// Los patrones "bloque X, (y?), Z" solo capturan 2 grupos (X y Z)
		if (match.length === 3) {
	...
	// Respaldo: quitar cualquier "(y?)" o "(...)" y buscar dos números con signo
	const limpio = raw.replace(/\([^)]*\)/g, " ");
	const nums = limpio.match(/-?\d+/g);
	if (nums && nums.length >= 2) {
		const x = Number(nums[0]);
		const z = Number(nums[1]);
		if (!Number.isNaN(x) && !Number.isNaN(z)) return { x, z };
	}
	return null;
}
...
function locateStructureNative(player, featureIds, options = {}) {
	...
	let best = null;
	let attempts = 0;
	// Ngân sách cứng số lần gọi getGeneratedStructures — chống crash máy yếu.
	const maxAttempts = intensive ? 220 : 140;
	const maxRadiusChunks = intensive ? 128 : 96;
	const stepChunks = intensive ? 4 : 5;
...
function tryLocateCommands(player, commands, fallback = {}) {
	...
	if (commandSucceeded && fallback.structureKey) {
		const wideEntity = locateStructureEntities(player, fallback.structureKey, { maxDistance: 1024 });
...
function locateAnyStructure(player) {
	...
	// Một lần quét entity duy nhất, map typeId -> structure key (rẻ hơn 18 lần getEntities).
	/** @type {Record<string, string>} */
	const typeToKey = {};
	for (const key of ANY_STRUCTURE_ORDER) {
	...
	let best = null;
	try {
		for (const ent of player.dimension.getEntities({ location: origin, maxDistance: 512 })) {
			if (!ent.isValid) continue;
