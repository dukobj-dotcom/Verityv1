...
function command(socket, command) {
  socket._lastCommand = command;
  write(socket, { type: "minecraftCommand", command, dimension_type: "overworld" });
}
...
function tryProcessRequest(socket, id, entry) {
  ...
  // NOTE: the previous `scriptevent hivemind:set remove` call is removed on
  // purpose. That command's `hivemindRequest<numeric-id>` argument contains
  // hyphens that Bedrock 1.26's Script Debugger command parser rejects, and
  // the debugger reacts by closing the TCP socket, which breaks the whole
  // request/response cycle before Groq can respond. Cleanup of the dynamic
  // properties is now done exclusively addon-side, either in the request
  // resolve path or in cleanupOrphanRequests() at world load.
  chat(request)
...
const tcp = net.createServer((socket) => {
  ...
  socket.on("error", (e) => console.log(`VERITY: socket error: ${e.message} code=${e.code || "?"}`));
  socket.on("close", (hadError) => {
    console.log(`VERITY: Minecraft disconnected hadError=${hadError} bytesWritten=${socket.bytesWritten} bytesRead=${socket.bytesRead} lastCommand=${socket._lastCommand ? JSON.stringify(String(socket._lastCommand).slice(0, 200)) : "(none)"}`);
  });
});
